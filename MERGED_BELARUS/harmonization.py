"""
Модуль гармонизации данных для проекта Scraping_Belarus_v2
...
def harmonize_availability(value: str, _store: str) -> str:
    """
    Гармонизация наличия товара для белорусских магазинов.

    Altagamma:
    - "На складе" / "В наличии" → "В наличии"
    - "Остаток 38.4 м2", "Остаток 10 шт" → "В наличии"
    - "Предзаказ" / "Предзаказ, поступление: DD.MM.YYYY" → "Под заказ"
    - None / пустое → "Нет в наличии"
    ...
    if 'в наличии' in value_lower or 'на складе' in value_lower:
        return 'В наличии'
    if value_lower.startswith('остаток'):  # "Остаток 38.4 м2"
        return 'В наличии'
    if 'распродажа' in value_lower:
        return 'В наличии'
    if 'предзаказ' in value_lower:
        return 'Под заказ'
    if 'под заказ' in value_lower or ('заказ' in value_lower and 'под' not in value_lower):
        return 'Под заказ'
    if 'нет' in value_lower or 'отсутств' in value_lower:
        return 'Нет в наличии'
