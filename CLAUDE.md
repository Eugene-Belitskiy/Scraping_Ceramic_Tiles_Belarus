...
**До-заливка недостающего в течение месяца:** каждый scrape-job перед запуском парсера восстанавливает
`data_*.json` из артефакта **предыдущего** запуска workflow (не текущего — `dawidd6/action-download-artifact`,
т.к. штатный `actions/download-artifact` видит только артефакты своего же запуска). Файл результата назван
по месяцу, поэтому в начале нового месяца восстанавливать нечего — шаг просто ничего не находит и работа
идёт с нуля. Если повторно нажать **Run workflow** в течение того же месяца (например, часть карточек не
собралась из-за rate-limit сайта), `already_done` внутри `get_data()` пропустит уже собранное и скрипт
дотянет только недостающее — не нужно вручную выбирать, что перезапускать.
